...
// ===== Камера рисования =====
// Источник — GameInfo/camera/graffiti.xml, прямой конфиг камеры. Раньше эти
// числа выводились окольно, по объектам ICameraControl_GRAFF на уровнях,
// и часть их оказалась про другое.
//
// `GlanceSideAngle` = `GlanceUpDownAngle` = 60 — сколько камере разрешено
// отвести взгляд. Прежние 30 брались с расставленных на уровнях камер, но те
// сценарные и срабатывают в отдельных местах, а ComponentGraffiti — рабочая
// камера, включающаяся всякий раз при рисовании. Здесь она и главнее.
// Заодно: 60 стоит во ВСЕХ камерах игры (ladder, ledge, subway, wallhug,
// pushpull), то есть это системная величина, а не настройка граффити.
export const CAMERA_CONE_DEG = 60;
export const CAMERA_TURN_RATE = 50;

// `CameraDistance` = 75 дюймов. Самая близкая камера в игре: у скрытности 85,
// у стены 90, у метро 100, у карниза 130, у лестницы и толкания 150.
export const CAMERA_DISTANCE = 75*0.0254;              // 1.905 м

// `FocalPoint Offset` (25, 60, 0) дюймов — точка, в которую камера смотрит,
// отсчитывается от игрока.
export const CAMERA_FOCUS_OFFSET = [25*0.0254, 60*0.0254, 0];   // (0.635, 1.524, 0) м

// `InterpolationDuration` = 0.2 с у граффити против 1.0 у всех остальных камер:
// эта камера встаёт впятеро быстрее любой другой.
export const CAMERA_BLEND_SECONDS = 0.2;

// Медленные дрейфы камеры рисования — это ComponentGraffiti, и только он:
// ни у одной другой камеры таких полей нет. `*Delay` читаем как период,
// `*Amount` — как размах.
// Оговорка: единицы `ShakeAmount` и `ZoomAmount` не установлены; крен и
// рыскание по смыслу в градусах.
export const CAMERA_DRIFT = {
  roll:  { period:5,  amount:1.25 },   // градусы
  yaw:   { period:10, amount:10   },   // градусы
  zoom:  { period:8,  amount:0.25 },   // доля расстояния
  shake: { period:0.75, amount:6  }    // при распылении
};
